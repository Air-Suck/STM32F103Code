...
;DELAY_NTIME		DCD		0X00000000
                ...
                ;ENTRY;ENTRY：程序入口点，程序的入口地址。程序的入口地址是一个标号，该标号必须在程序中定义，且必须是全局可见的。
;__main		    PROC
				;EXPORT __main
                ;BL Clock_Init;设置时钟：选择外部时钟源，设置PLL倍频系数，设置AHB时钟分频系数，设置APB1时钟分频系数，设置APB2时钟分频系数
...
;Clock_Init		PROC;时钟初始化
                ...
                ;ORR R1,R1,#0x10000;将RCC_CR的第16位设置为1,即是外部时钟源
...
;SYSTICK_INIT    PROC
                ...
                ;ORR R1,R1,#0x2;开启中断
                ;STR R1,[R0]
                ;;设置重装载值为0x2904，即是10.5MHz/1000=10500Hz，1ms中断一次
                ;LDR R0,=STK_LOAD
                ;LDR R1,=0xA037A0
                ;STR R1,[R0]
                ;POP {R0,R1,PC}
